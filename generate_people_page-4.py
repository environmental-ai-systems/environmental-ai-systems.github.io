...
def generate_alumni_card(person):
    card = '            <div class="alumni-card">\n'
    card += f'                <div class="alumni-name">{person["name"]}</div>\n'
    if person.get('former_role'):
        card += f'                <div class="alumni-former-role">{person["former_role"]}</div>\n'
    if person.get('current_position'):
        card += f'                <div class="alumni-current"><strong>Now:</strong> {person["current_position"]}</div>\n'
    if person.get('url'):
        card += f'                <div class="person-contact"><a href="{person["url"]}" target="_blank">Profile →</a></div>\n'
    card += '            </div>\n'
    return card
